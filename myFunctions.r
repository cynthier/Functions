...
"1_hNP_day9"= "#bebada", 
"2_HDAC1i_day9"="#4000FF", 
	"3_hNP_day20"="#fcd90c",
	"4_HDAC1i_day20"="#f75716",
	"5_HDAC1_2_KD_day20"="#8a1518",
  ...
  # "neuronal progenitor"         = "#1f77b4", # 橙色
  "neuronal progenitor"         = "#c0dffb", # 橙色

  # "cycling neuroblast"          = "#34634c", # 金黄色
  # "neuroblast1"          = "#34634c", # 金黄色
	# "neuroblast2"="#3e9621",
	# "neuroblast3"="#98df8a",
	# "neuroblast6"='#cddfc7',
	# "neuroblast5" = "#7ed957",  # 新增绿色 (Green)
	# "neuroblast4" = "#21c977",  # 新增绿色 (Green)

  # "neuroblast1" = "#5279a7",  # 深墨绿 (Deep Forest Green)：极深的暗绿，作为基础色压住阵脚，对比度极强。
  # "neuroblast2" = "#2b8cbe",  # 青翠绿 (Mint/Cyan-Green)：保留了你的原色。偏蓝调的明亮绿，非常抢眼。
  # "neuroblast3" = "#7bccc4",  # 经典草绿 (Grass Green)：保留了你的原色。最正的自然绿色。
  # "neuroblast4" = "#ccebc5",  # 亮黄绿 (Apple/Lime Green)：保留了你的原色。偏黄调，荧光感强，用来标注最小或最重要的亚群。
  # "neuroblast5" = "#a1d99b",  # 橄榄绿 (Olive Green)：带一点泥土/褐色调的绿。因为色相特殊，与其他纯绿色的区分度极高。
  # "neuroblast6" = "#7eb179",  # 柔和豆绿 (Pastel Green)：取代了你原本太白的 `#cddfc7`。这个颜色足够浅，但又不会融入白色背景，适合细胞数最多的大群。

  "cycling neuroblast" = "#005824",  # 深墨绿 (最深、最冷)
  "neuroblast1" = "#238B45",  # 海藻绿
  "neuroblast2" = "#41AB5D",  # 翠绿
  "neuroblast3" = "#7ED957",  # 黄绿 (开始转暖)
  "neuroblast4" = "#ADDD8E",  # 嫩绿
  "neuroblast5" = "#D9F0A3",
  "HDAC1_KD" = "red",
  "neuroblast_branchA_early" = "#238B45",
  "neuroblast_branchA_late" = "#7ED957",
  "neuroblast_branchB" = "#D9F0A3",

  "ENC(GAD2_SLC6A5)(day20)" = "#ff9896",
  "IN?(NTNG1_PAX5)(day20)" = "#c49c94",
  "Neuronal progenitor" = "#c0dffb",
  "Neuroblast"="#a9dd98", 
  'IPAN/IN(NTNG1,KCNJ6)' ="#d8b4f9",
  'IPAN/IN(NTNG1,GALNTL6)'= "#8181f7",
  'IPAN/IN(NTNG1)'= "#9966FF",
  'IN(GAD2)' = "#ff9896",
  'IN(SST)' ="#ffcfce",

  "1_Neuronal progenitor" = "#c0dffb",
  "2_Neuroblast"="#a9dd98", 
  '4_IPAN/IN(NTNG1,KCNJ6)' ="#d8b4f9",
  '5_IPAN/IN(NTNG1,GALNTL6)'= "#8181f7",
  '3_IPAN/IN(NTNG1)'= "#9966FF",
  '6_IN(GAD2)' = "#ff9896",
  '7_IN(SST)' ="#ffcfce",

  "ENC(KCNJ6_PHOX2B)"           = "#a9bcf5", # 天蓝色
  "ENC(KCNJ6)"           = "#dbcb98", # 天蓝色

  "IN?(NTNG1_PAX5)"             = "#c49c94", # 宝蓝色
  "IPAN/IN(LHX1_GALNTL6)"       = "#9966FF", # 薰衣草紫
  "ENC(ST18_high)"              = "#dce14f", # 亮紫色
  "ENC(GAD2_SLC6A5)"            = "#ff9896", # 桃粉色
  "ENC(PAX2_SLC6A5)"            = "#ff9896", # 桃粉色


   "neuroblast_branchA_early(day9)" = "#238B45",
   "neuroblast_branchA_late(day20)" = "#7ED957",

  "ENC(ADCY8_HES6)"             = "#F7BE81", # 玫红色
  "ENC(ISL1_SST)"               = "#00BFFF", # 深天蓝
  "ENC(C12orf57)"               = "#e2a9f3",  # 棕红色 (特别标注你关注的 C12orf57)

  "1_cycling neuronal progenitor" = "#1f77b4", # 亮红色 (代表活跃分裂)
  "2_neuronal progenitor"         = "#c0dffb", # 橙色
  "3_cycling neuroblast"          = "#34634c", # 金黄色
	"4_neuroblast1"="#3e9621",

	"5_neuroblast2"="#98df8a",
	"6_neuroblast3"='#cddfc7',
  "7_ENC(KCNJ6_PHOX2B)"           = "#a9bcf5", # 天蓝色
  "7_ENC(KCNJ6)"           = "#a9bcf5", # 天蓝色

  "8_IN?(NTNG1_PAX5)"             = "#c49c94", # 宝蓝色
  "9_IPAN/IN(LHX1_GALNTL6)"       = "#9966FF", # 薰衣草紫
  "10_ENC(ST18_high)"              = "#dce14f", # 亮紫色
  "11_ENC(GAD2_SLC6A5)"            = "#ff9896", # 桃粉色
  "12_ENC(ADCY8_HES6)"             = "#F7BE81", # 玫红色
  "13_ENC(ISL1_SST)"               = "#00BFFF", # 深天蓝
  "14_ENC(C12orf57)"               = "#e2a9f3",  # 棕红色 (特别标注你关注的 C12orf57)
...
"Neural crest" = "#85be72",
  ...
  "neuroblast_1_early"='#3e9621',
  "neuroblast_1_early(day9)"='#3e9621',
  "neuroblast_1_early(day20)"='#D9F0A3',
  "neuroblast_1_late"='#98df8a',
  "neuroblast_2"='#D9F0A3',
...
newseurat2_func <- function(obj) {
  mt <- GetAssayData(obj, layer = "counts") %>% as.matrix()
  rownames(mt) <- make.unique(toupper(rownames(mt)))
  obj <- CreateSeuratObject(counts = mt, meta.data = obj@meta.data)
  return(obj)
...
my_volcan.plt <- function (meta, cutoff_lfc, cutoff_qval, x, rev.result = FALSE, 
    y, label = TRUE, label.var = "label", max.overlaps = 20, 
    title = "DEGs", label.size = 2.5, color.var = "avg_log2FC") 
{
    library(ggrepel)
    library(ggplot2)
    if (rev.result) {
    ...
    meta$log10_qval <- -log10(meta[, y])
    meta$log10_qval <- ifelse(meta$log10_qval > 100, 100, meta$log10_qval)
    down.temp <- meta %>% dplyr::filter(.data[[x]] < -cutoff_lfc & 
        .data[[y]] < cutoff_qval)
    up.temp <- meta %>% dplyr::filter(.data[[x]] > cutoff_lfc & 
        .data[[y]] < cutoff_qval)
    label.data <- data.frame(num = c(nrow(down.temp), nrow(up.temp)), 
        x = c(min(down.temp[, x]) * 0.8, max(up.temp[, x]) * 
            0.8), y = rep(max(c(max(down.temp$log10_qval) * 0.8, 
            max(up.temp$log10_qval) * 0.8), 2)), col = c("blue", 
            "red"))
    if (label) {
        p <- ggplot(meta, aes(x = .data[[x]], y = log10_qval)) + 
            geom_hline(yintercept = -log10(cutoff_qval), linetype = "dashed", 
                color = "#999999") + geom_vline(xintercept = c(-cutoff_lfc, 
            cutoff_lfc), linetype = "dashed", color = "#999999") + 
            geom_point(size = 1, color = ifelse(meta[, color.var] < 
                -cutoff_lfc & meta[, "log10_qval"] > -log10(cutoff_qval), "blue", ifelse(meta[, color.var] > 
                cutoff_lfc & meta[, "log10_qval"] > -log10(cutoff_qval), "red", "gray"))) + geom_text_repel(data = meta, 
            label = meta[, label.var], max.overlaps = max.overlaps, 
            size = label.size, box.padding = 0, nudge_x = 0.05, 
            nudge_y = 0.1) + theme_bw(base_size = 14) + ggsci::scale_color_jama() + 
            geom_text(data = label.data, aes(x = x, y = y, label = num), 
                color = ifelse(label.data$col == "red", "red", 
                  "blue"), size = 6) + theme(panel.grid = element_blank(), 
        ...
        p <- ggplot(meta, aes(x = .data[[x]], y = log10_qval)) + 
            ...
            geom_point(size = 1, color = ifelse(meta[, color.var] < 
                -cutoff_lfc & meta[, "log10_qval"] > -log10(cutoff_qval), "blue", ifelse(meta[, color.var] > 
                cutoff_lfc & meta[, "log10_qval"] > -log10(cutoff_qval), "red", "gray"))) + theme_bw(base_size = 14) + 
            ggsci::scale_color_jama() + geom_text(data = label.data, 
            aes(x = x, y = y, label = num), color = ifelse(label.data$col == 
                "red", "red", "blue"), size = 6) + theme(panel.grid = element_blank(), 
...
my_box_compare.plt <- function(
  ...
  library(ggsignif)
  library(dplyr)
  label_y_positions = rep(max(meta[[y]], na.rm = TRUE) * 1.05, length(comparison))
...
integrated_func <- function (obj, split.var = "group", split.obj = TRUE, dims = 20,  nfeatures = 2000) 
{
    library(Seurat)
    options(future.globals.maxSize = 1e+14)
    if (split.obj) {
        obj[["RNA"]] <- split(obj[["RNA"]], obj@meta.data[, split.var])
        obj <- NormalizeData(obj, verbose = FALSE) %>% FindVariableFeatures(verbose = FALSE, nfeatures = nfeatures) %>% 
        ...
        obj <- IntegrateLayers(object = obj, method = RPCAIntegration, 
            ...
            verbose = FALSE)
        obj <- FindNeighbors(obj, reduction = "integrated.rpca", dims = 1:dims)
        obj <- RunUMAP(obj, reduction = "integrated.rpca", dims = 1:dims, 
            reduction.name = "umap.rpca")
        obj <- RunTSNE(obj, reduction = "integrated.rpca", dims = 1:dims, 
            reduction.name = "tsne.rpca")

        obj <- FindNeighbors(obj, reduction = "integrated.harmony", dims = 1:dims,verbose = FALSE)
        obj <- RunUMAP(obj, reduction = "integrated.harmony", 
            dims = 1:dims, reduction.name = "umap.harmony",verbose = FALSE)
        obj <- RunTSNE(obj, reduction = "integrated.harmony", 
            dims = 1:dims, reduction.name = "tsne.harmony", verbose = FALSE)
    }
    else {
        obj <- NormalizeData(obj, verbose = FALSE) %>% FindVariableFeatures(nfeatures = nfeatures, verbose = FALSE) %>% 
            ScaleData(verbose = FALSE) %>% RunPCA(verbose = FALSE)
        ElbowPlot(obj)
        obj <- IntegrateLayers(object = obj, method = HarmonyIntegration, 
            orig.reduction = "pca", new.reduction = "integrated.harmony", 
        ...
        obj <- RunTSNE(obj, reduction = "integrated.rpca", dims = 1:dims, 
            reduction.name = "tsne.rpca", verbose = FALSE)
...
dotplt.enriched <- function(data = data, x = "Cluster", y = "Description", size = "Count", color = "p.adjust", title){ 
    library(ggplot2)
  ...
  theme(axis.text = element_text(size = 12, color = "black")) + ggtitle(title) + xlab(NULL) + ylab("Pathways") + scale_size(range = c(4,7)) + 
    theme(axis.text = element_text(size = 14), 
          ...
          legend.text = element_text(size = 12), 
          legend.title = element_text(size = 14)
          ) + scale_color_gradient(low = "#DF3A01", high = "#F6D8CE") & labs(y = NULL);
